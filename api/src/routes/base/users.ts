...
import users, { Activity } from "../../database/models/users";
...
// GET /api/users/:id/svg
router.get("/users/:id/svg", async (req: Request, res: Response) => {
    try {
        const id = req.params.id;
        ...
        const status = data.current ? { type: "current", ...data.current } : { type: "recent", ...data.recent };
        ...
        const svgContent = `
        <svg xmlns="http://www.w3.org/2000/svg" width="400" height="400" xmlns:xlink="http://www.w3.org/1999/xlink">
                        ...
                        <div style="width: 100%; display: flex; flex-direction: row; justify-content: start; align-items: center; gap: 1rem;">
                            ...
                            <div style="display: flex;flex-direction:column;justify-content:start;align-items:start;gap:0;">
                                ...
                                <h4 style="display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;  overflow: hidden; margin: 0; padding: 0; font-weight: 400;">
                                    ${format(line2 as string, status)}
                                </h4>

                                <h4 style="display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;  overflow: hidden; margin: 0; padding: 0; font-weight: 400;">
                                    ${format(line3 as string, status)}
                                </h4>
